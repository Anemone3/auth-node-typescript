...
export class AuthController{
    ...
    constructor(
     ...
     registerUser = async(req:Request,res:Response,next:NextFunction): Promise<any> => {
        ...
        try {
            ...
            if(!usuarioRegister) return res.status(401).json({error: 'Invalid credentials, service'})
    ...
    loginUser =  async(req:Request,res:Response,next:NextFunction): Promise<any> => {
        ...
        try {
            ...
            if(!userLogging) return res.status(401).json({error: 'Invalid credentials, service'})
    ...
    verificateUserAuth = async(req:Request,res:Response,next:NextFunction): Promise<any> => {
        const { otp,email,id,role } = req.body;   

        try {

            if(!otp || !email || !id || !role) return res.status(400).json({error: 'Invalid body, service'})

            const result = await this.authService.verifiyUserAndSendToken(email,otp,id,role);
            res.status(200).json({
                ...result
            })
        } catch (error) {
